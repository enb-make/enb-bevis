...
module.exports = inherit({
    ...
    __constructor: function (sources, formats) {
        ...
        this._formats.forEach(function (format) {
            var suffix = format.getSuffix();
            this._formatIndex[suffix] = format;

            this._suffixes.push(suffix);
        }, this);
    },
    ...
    normalizeDep: function (dep, blockName) {
        ...
        if (typeof dep === 'string') {
            ...
            var preprocessDep = function (subDep) {
                if (!subDep.block) {
            ...
            var buildModDecls = function (decl, mods) {
                if (mods) {
            ...
            if (dep.suffix && this._suffixes.indexOf(dep.suffix) === -1) {
                decl.suffix = dep.suffix;
            }
            var res = [];
            if (res.elems) {
    ...
    resolve: function () {
        ...
        if (items.length) {
            ...
            throw new Error('Unresolved deps: \n' + errorMessage.join('\n'));
        }
        return result;
    }
});
