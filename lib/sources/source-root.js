...
var SourceRoot = module.exports = inherit({
    ...
    addFile: function (filename, blockName, elemName, modName, modValue) {
        ...
        var block = hasOwnProperty(this._blocks, blockName) ?
            this._blocks[blockName] :
            (this._blocks[blockName] = {name: blockName, files: [], dirs: [], elements: {}, mods: {}});
        var destElement;
        if (elemName) {
            destElement = hasOwnProperty(block.elements, elemName) ?
                block.elements[elemName] :
                (block.elements[elemName] = {name: elemName, files: [], dirs: [], mods: {}});
        } else {
            destElement = block;
        }
        var collectionKey = fileInfo.isDirectory ? 'dirs' : 'files';
        if (modName) {
            var mod = hasOwnProperty(destElement.mods, modName) ?
                destElement.mods[modName] :
                (destElement.mods[modName] = {});
            var modValueFiles = (hasOwnProperty(mod, modValue) ?
                mod[modValue] :
                (mod[modValue] = {files: [], dirs: []})
            )[collectionKey];
